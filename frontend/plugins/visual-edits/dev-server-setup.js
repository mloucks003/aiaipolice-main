      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="dev@lawenforcement-rms.local" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="dev@lawenforcement-rms.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
