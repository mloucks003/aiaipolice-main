    ...
    try {
      ...
      this.ws.onmessage = (event) => {
        try {
          ...
          // Normalize backend messages into {type, payload, timestamp} format
          const msgType = raw.type;
          const timestamp = raw.timestamp || Date.now();
          
          // Extract payload by removing type and timestamp from the raw message
          const {type: _t, timestamp: _ts, ...payload} = raw;
